...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    ...
    def step(self, action):
        ...
        if self.reward_type == 'sparse':
            reward = 1.0 if np.linalg.norm(ob[0:2] - self.target) <= 0.5 else 0.0
        elif self.reward_type == 'dense':
            reward = np.exp(-np.linalg.norm(ob[0:2] - self.target))
        else:
    ...
    def set_marker(self):
        self.data.site_xpos[self.model.site_name2id('target_site')] = np.array([self.target[0]+1, self.target[1]+1, 0.0])

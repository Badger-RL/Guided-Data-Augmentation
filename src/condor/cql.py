                ...
                #     aug = f'{aug}_{expert}'
                    ...
                    for gap in [5]:
                            for hd in [256]:
                                ...
                                save_dir = f'results/{env_id}/{aug}/cql/nl_{nl}/hd_{hd}/gap_{gap}'
                                hidden_dims = 256

                                max_timesteps = int(0.3e6) if nl == 2 else int(0.3e6)
                                eval_freq = int(20e3)
                                ...
                                command = f'python -u algorithms/cql.py --max_timesteps {max_timesteps} --eval_freq {eval_freq}' \
                                          ...
                                          f' --n_layers {nl} --hidden_dims {hidden_dims}'# --device cuda'
                                          # f' --qf_lr {qf_lr}' \

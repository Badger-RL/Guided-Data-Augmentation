...
class KeepawayEnv(BaseEnv):
    ...
    def _observe_state(self):
        ...
        distance = np.linalg.norm(np.array([self.target_x, self.target_y]))
        ...
        # Add robot positions in form [distance from center, angle]
        for i in range(self.num_agents):
            distance = np.linalg.norm(np.array([self.robot_x[i], self.robot_y[i]]))
            angle = math.atan2(self.robot_y[i], self.robot_x[i])
            state.extend([
                distance,
                angle,
            ]
        ...
        # Add adversary positions in form [distance from center, angle]
        for i in range(self.num_adversaries):
            distance = np.linalg.norm(np.array([self.adversary_x[i], self.adversary_y[i]]))
            angle = math.atan2(self.adversary_y[i], self.adversary_x[i])
            state.extend([
                distance,
                angle,
            ]

...
class RotateReflectTranslate(AbstractSimAugmentationFunction):
    ...
    def _is_valid_input(self, absolute_obs, absolute_next_obs):
        if self.at_goal(absolute_obs[2], absolute_obs[3]) or self.at_goal(absolute_next_obs[2], absolute_next_obs[3]):
            return False
        return True

    def _rotate(self, aug_absolute_obs, aug_absolute_next_obs):
    ...
    def _reflect(self, aug_absolute_obs, aug_absolute_next_obs, aug_action):
    ...
    def _translate(self, aug_absolute_obs, aug_absolute_next_obs):
    ...
    def _augment(self,
        ...
        aug_absolute_obs = copy.deepcopy(abs_obs)
        aug_absolute_next_obs = copy.deepcopy(abs_next_obs)
        aug_action = action.copy()
        aug_done = done.copy()

        self._rotate(aug_absolute_obs, aug_absolute_next_obs)
        if np.random.random() < 0.5:
            self._reflect(aug_absolute_obs, aug_absolute_next_obs, aug_action)
        self._translate(aug_absolute_obs, aug_absolute_next_obs)

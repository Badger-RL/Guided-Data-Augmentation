...
class PointMazeAugmentationFunction(AugmentationFunctionBase):
    def __init__(self, env, **kwargs):
        ...
        for theta in self.thetas:
            ...
            M = torch.from_numpy(M).type(torch.float32)
            self.rotation_matrices.append(M)
        self.target = torch.Tensor(self.env.get_target())
        self.wall_locations = []
    ...
    def _is_valid_position(self, xy):
        x, y = xy[:, 0], xy[:, 1]
        is_valid_position = torch.zeros_like(x).type(torch.bool)
        for box_location in self.wall_locations:
            mask = self._is_in_wall(box_location, x, y)
            is_valid_position |= mask
    ...
    def _reward(self, next_obs):
        ...
        if self.env.reward_type == 'sparse':
            reward = torch.zeros(len(next_obs))
            at_goal = np.linalg.norm(next_obs[:, 0:2] - self.target, axis=-1) < 0.5
            reward[at_goal] = 1
    ...
    def augment(self,
        ...
        n = len(obs)
        aug_obs = torch.clone(obs)
        aug_next_obs = torch.clone(next_obs)

        # translate
        delta = torch.from_numpy(self._sample_pos(n).T)
        aug_obs[:, :2] = delta
        ...
        # rotate action
        aug_action = (M @ action.T).T

        # rotate next_obs about obs
        rotated_delta_obs = (M @ delta_obs[:, :2].T).T
        aug_next_obs[:, :2] = aug_obs[:, :2] + rotated_delta_obs

        # rotate velocity
        aug_obs[:, 2:] = (M @ aug_obs[:, 2:].T).T
        aug_next_obs[:, 2:] = (M @ aug_next_obs[:, 2:].T).T
...
class PointMazeGuidedAugmentationFunction(PointMazeAugmentationFunction):
    ...
    def _sample_rotation_matrix(self, obs, next_obs, **kwargs):
        ...
        location = np.array([np.round(x+self.agent_offset).astype(int), np.round(y+self.agent_offset).astype(int)]).T
        guide_thetas = [self.guide_thetas[tuple(loc)] for loc in location]
        guide_theta = np.array([[np.random.choice(guide_thetas_i)] for guide_thetas_i in guide_thetas])

        # thetas = np.tile(self.thetas.reshape(-1, 1), reps=(1,4))
        theta = theta.reshape(-1, 1)
        aug_thetas = (self.thetas + theta)%(2*np.pi)
        dist = np.abs(guide_theta - aug_thetas)
        idx = np.argmin(dist, axis=-1).astype(int)

        M = self.rotation_matrices[idx]
        M = torch.from_numpy(M).type(torch.float32)
    ...
    def augment(self,
        ...
        # rotate action
        aug_action = torch.einsum("nbh,nb->nh", (M, action))
        # aug_action = (M @ action.T).T

        # rotate next_obs about obs
        rotated_delta_obs = torch.einsum("nbh,nb->nh", (M, delta_obs[:, :2]))
        aug_next_obs[:, :2] = aug_obs[:, :2] + rotated_delta_obs

        # rotate velocity
        aug_obs[:, 2:] = torch.einsum("nbh,nb->nh", (M, aug_obs[:, 2:]))
        aug_next_obs[:, 2:] = torch.einsum("nbh,nb->nh", (M, aug_next_obs[:, 2:]))

...
class PointMazeGuidedTrajectoryAugmentationFunction(PointMazeGuidedAugmentationFunction):
    ...
    def augment_fixed_origin(self, 
        ...
        if not self.is_in_valid_boundaries(next_obs[:2]) or not self.is_in_valid_boundaries(obs[:2]):
    ...
    def augment_trajectory(self, trajectory: dict):
        ...
        new_origin, _ = self._sample_pos()
        delta_obs = - trajectory['observations'][0] + trajectory['next_observations'][-1]
        new_end = new_origin + delta_obs[:2]
        M = self._sample_rotation_matrix(new_origin, new_end)
        ...
        while i < length:
            if is_new_trajectory:
                # M = self._sample_rotation_matrix()
                new_origin, _ = self._sample_pos()
                delta_obs = - trajectory['observations'][i] + trajectory['next_observations'][-1]
                new_end = new_origin + delta_obs[:2]
                M = self._sample_rotation_matrix(new_origin, new_end)
            else:
                new_origin = augmented_next_obs[:2].copy()
            ...
            augmented_obs[:2] = new_origin.copy()
            delta_pos = next_observation[:2] - observation[:2]
            rotated_delta_pos = M.dot(delta_pos).T
            augmented_next_obs[:2] = augmented_obs[:2].copy() + rotated_delta_pos
            augmented_action = M.dot(action).T
            augmented_obs[2:] = M.dot(observation[2:]).T
            augmented_next_obs[2:] = M.dot(next_observation[2:]).T
            augmented_reward = self._reward(augmented_next_obs)
            if self.is_in_valid_boundaries(augmented_obs[:2]) and self.is_in_valid_boundaries(augmented_next_obs[:2]):
                # print(f'{observation[:2]} -> {augmented_obs[:2]}')
    ...
    def augment_trajectory1(self, trajectory: dict):
        length = len(trajectory['observations'])
        ...
        for i in range(length):
            ...
            if self.is_in_valid_boundaries(augmented_obs) and self.is_in_valid_boundaries(augmented_next_obs):
                is_new_trajectory = False
                augmented_trajectory['observations'].append(augmented_obs)
                augmented_trajectory['actions'].append(augmented_action)
                augmented_trajectory['rewards'].append(augmented_reward)
